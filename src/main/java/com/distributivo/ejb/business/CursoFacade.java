...
public class CursoFacade extends AbstractFacade<Curso> {
    ...
    @Override
    public InfoTable<Curso> loadTable(Integer first, Integer pageSize, List<String> search, String addquery, Map<String, Object> parametros) {
        StringBuilder sb = new StringBuilder();
        sb.append("SELECT c FROM Curso c where c.estado = 'ACTIVO'");
        ...
        InfoTable<Curso> info = new InfoTable();
    ...
    public List<Curso> buscarPorCoincidencia(String filter) {
        filter = "%" + filter + "%";
    ...
    public List<Curso> buscarPorEstado() {
        Query q = getEntityManager().createQuery("SELECT l FROM Curso l WHERE l.estado='ACTIVO'");
        return q.getResultList();
    }
    public List<Curso> buscarPorCodigo(Integer codigo) {
        Query q = getEntityManager().createQuery("SELECT l FROM Curso l WHERE l.idCurso =  " + codigo + " AND l.estado='ACTIVO'");
        q.setMaxResults(10);
        return q.getResultList();
    }
}
