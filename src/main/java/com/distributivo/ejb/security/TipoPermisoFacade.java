...
public class TipoPermisoFacade extends AbstractFacade<TipoPermiso> {
    ...
    @Override
    public InfoTable<TipoPermiso> loadTable(Integer first, Integer pageSize, List<String> search,String addquery,Map<String,Object> parametros) {
       StringBuilder sb = new StringBuilder();
        sb.append("SELECT u FROM TipoPermiso u ");
        if (search.size() > 0) {
            sb.append("WHERE ");
            sb.append("(");
            for (int i = 0; i < search.size(); i++) {
                sb.append("(");
                sb.append(" UPPER(u.nombreTiper) like :dato").append(i);
        ...
        InfoTable<TipoPermiso> info = new InfoTable();
