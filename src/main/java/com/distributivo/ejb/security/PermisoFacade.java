...
public class PermisoFacade extends AbstractFacade<Permiso> {
    ...
    public List<Permiso> getPermisosPadre() {
        return this.getResultListNameQuery("Permiso.findByPadre");
    }

    @Override
    public InfoTable<Permiso> loadTable(Integer first, Integer pageSize, List<String> search,String addquery,Map<String,Object> parametros) {
        StringBuilder sb = new StringBuilder();
        sb.append("SELECT u FROM Permiso u ");
        if (search.size() > 0) {
            sb.append("WHERE ");
            sb.append("(");
            for (int i = 0; i < search.size(); i++) {
                sb.append("(");
                sb.append(" UPPER(u.nombrePerm) like :dato").append(i);
                sb.append(" OR UPPER(u.detallePerm) like :dato").append(i);
        ...
        InfoTable<Permiso> info = new InfoTable();
